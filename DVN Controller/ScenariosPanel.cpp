...
ScenariosPanel::ScenariosPanel(wxWindow* parent, Load* load) : ScenariosPanel(parent)
{
	for (DVNFileData* scen : load->children) {
		AddScenario(dynamic_cast<Scenario*>(scen));
	}
}
...
ScenariosPanel::ScenariosPanel(wxWindow* parent) : wxPanel(parent)
{
	...
	scenSizer->AddSpacer(FromDIP(38));
...
{
	SideMenuCtrl* scenCtrl = new SideMenuCtrl(scenList, this, scen, false); //Implement subMenu
...
void ScenariosPanel::ChangeLoad(Load* load)
{
	this->load = load;
	for (char i = 0; i < SCENARIOS_COUNT; i++)
	{
		scenarios[i]->ChangeSource(load->children[i]);
		ChangeSelection(scenarios[i]);
	}
}
...
void ScenariosPanel::ChangeSelection(SideMenuCtrl* scenCtrl)
{
	...
	bandsPanel->ChangeSource(dynamic_cast<Scenario*>(curScen->GetSource()));
